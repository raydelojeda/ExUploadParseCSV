...
$('#the-basics .typeahead').bind('typeahead:open', function(ev, suggestion) {
...
$('#the-basics .typeahead').bind('typeahead:render', function(event, suggestions, async, dataset) {
    if( suggestions != undefined && $('.tt-all-results').length <= 0){ // only add if suggestions are available and custom element not already present
        $('.tt-dataset').append( '<div class="tt-suggestion tt-selectable tt-all-results">Show all results</div>');
    }
});
